...
public class TutorViewerService {
    ...
    public List<com.tutorlink.matchmaking_domain.tutordomainmanager.tutorprofileretrieval.model.dto.resp.TutorProfileResp> getAllTutors() {
        ResponseEntity<List<com.tutorlink.matchmaking_domain.tutordomainmanager.tutorprofileretrieval.model.dto.resp.TutorProfileResp>> response = tutorViewClient.getAllTutorProfiles();
        return response.getBody();
    }

    public TutorProfileResp getTutorProfileById(Long tutorId) {
        ResponseEntity<com.tutorlink.matchmaking_domain.tutordomainmanager.tutorprofileretrieval.model.dto.resp.TutorProfileResp> response = tutorViewClient.getTutorProfile(tutorId);
        return response.getBody();
    }

    public ConnectionResp createConnection(CreateConnectionReq connectionReq) {
        //not 100% sure if this goes here, but I'm assuming students can also create connections from tutor's profiles
        return null;
    }
}
