...
public class SecurityConfig {
    ...
    public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http) {
        ...
        http.addFilterBefore(corsWebFilter(), SecurityWebFiltersOrder.CORS) // Add the CorsFilter before security
                                                                            // filters
                .authorizeExchange(auth -> {
                    ...
                    if (!authConfig.isCheckAuthentication()) {
                        auth.pathMatchers("/gateway/**").permitAll(); // Allow all requests on gateway paths if
                ...
                .csrf(csrf -> csrf.disable()); // Disabling CSRF for the Gateway, assuming you're using token-based auth
                                               // or similar
